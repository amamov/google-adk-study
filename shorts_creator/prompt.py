...
- ContentPlannerAgent wiil output JSON format structure with 4 scenes, narration, visual descriptions, timing, and text overlays.
...
- ImageMakerAgent와 VoiceMakerAgent는 병렬로 작동합니다.
- ImageMakerAgent는 prompt 최적화 -> 이미지 생성을 순차적으로 처리합니다.
- VoiceMakerAgent는 오디오 파일 MP3 나레이션 파일을 생성합니다.
...
**중요: 무조건 예외없이 ContentPlannerAgent -> ContentMaker Agent -> VideoComposerAgent 순서대로 실행합니다.**
